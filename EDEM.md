...
3) 사실적인 재료 거동 - 유재은   
   EDEM은 Altair에서 개발한 고체 입자 시뮬레이션 소프트웨어로, 분체 및 벌크 소재의 흐름을 현실적으로 분석하고 예측하기 위해 다양한 산업 분야에서 사용된다. 이 소프트웨어는 Discrete Element Method(DEM, 개별 요소법)를 기반으로 하며, 입자 간의 충돌, 접촉, 마찰, 점착 등 다양한 물리적 상호작용을 정밀하게 계산하여 실제 재료의 거동을 가상 환경에서 사실적으로 재현할 수 있다.
   EDEM 시뮬레이션은 신뢰성 높은 수치 해석 기법과 산업 등급의 고성능 다면체 파티클 솔버를 기반으로 하여, 다양한 입자 기반 재료를 정밀하게 모델링할 수 있도록 설계된 고급 시뮬레이션 도구이다. 이 소프트웨어는 분말, 섬유, 입자, 정제, 알갱이 등 실제 산업에서 사용되는 다양한 고체 재료의 동적 특성을 가상 환경에서 구현할 수 있으며, 미세 입자의 흐름부터 큰 입자의 압축 및 충돌까지 폭넓은 물리 현상을 다룰 수 있다. 특히 고압 상황, 점성 유체와의 상호작용, 또는 복잡한 재료 간 마찰 및 결합 등 다양한 조건에서도 안정적인 해석 결과를 제공한다.
